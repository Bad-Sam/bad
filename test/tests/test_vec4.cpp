...
void test_vec4_dot()
{
    vec4 a0 = f32x4_set(snanf,   .0f,  1.f, -2.f);
    vec4 a1 = f32x4_set(inff,    .0f,  1.f, -2.f);
    vec4 a2 = f32x4_set(-inff,   .0f,  1.f, -2.f);
    vec4 a3 = f32x4_set(denormf, .0f,  1.f, -2.f);
...
void test_vec4_length_squared()
{
    ...
    bad_test_check(is_qnan(length_squared0_store)
                && length_squared1_store == inff
                && length_squared2_store == inff
...
void test_vec4_lerp()
{
    ...
    bad_test_check(is_qnan(lerp0_store[0])                 && lerp0_store[1] == 2.5f
                ...
                && lerp2_store[0] == -inff                 && lerp2_store[1] == 2.5f
                && lerp2_store[2] == -.5f                  && lerp2_store[3] == -.5f
                && lerp3_store[0] == (denormf * .5f + 5.f) && lerp3_store[1] == 2.5f
